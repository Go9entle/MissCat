...
#'
#' The difference between this function and the impute4mmv function
#' is that this function focuses on not changing the order of the interpolation results,
#' and can be used to compare the accuracy with other functions.
#'
...
#' imputed_result <- impute5mmv(dat, method = "EM", conj_prior = "non.informative")
#'
#' imputed_result <- impute5mmv(dat, method = "DA", conj_prior = "non.informative")
#' @export
impute5mmv <- function(dat, method = c("EM", "DA"),
  ...
  dat2 <- impute_multinomial_all(dat, mle_multinomial@mle_x_y, p=p)
  ...
  ret <- methods::new("imputeMulti",
                      ...
                      data = list(missing_data = dat_miss, imputed_data = dat2),
                      nmiss = nrow(dat_miss)
  )
